...
public class TypePresenter extends BasePresenter<TypeView> {
    ...
    public <T>void getGoodsFromF(String id,Class<T> t){
     ...
     public  <T> void  getGoodsBuyNowFromF(Class<T> t,String key,String cart_id, String ifcart){

          HttpUtils.getBuyGoodsData(key,cart_id,ifcart, new HttpUtils.CallBackDataF<BuyNowBean>() {
               @Override
    ...
    public  <T> void  getOrderDataFromF(Map<String, Object> map ){

        HttpUtils.getOrderData(map,new HttpUtils.CallBackDataF<OrderBean>() {
            @Override
            public void callBackF(OrderBean homeBean) {
                getIView().getOrderData(homeBean);
            }
        });
    }

    public  <T> void  getPayInfoDataFromF(Map<String, Object> map ){

        HttpUtils.getPayInfoData(map,new HttpUtils.CallBackDataF<PayMentInfo>() {
            @Override
            public void callBackF(PayMentInfo homeBean) {
                getIView().getPayList(homeBean);
            }
        });
    }
    public  <T> void  getPayByPredeposit(Map<String, Object> map ){

        HttpUtils.getPredeposit(map,new HttpUtils.CallBackDataF<PredepositBean>() {
            @Override
            public void callBackF(PredepositBean homeBean) {
                getIView().getPredeposit(homeBean);
            }
        });
    }
